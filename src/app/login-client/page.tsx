...
export default function LoginClient() {
  ...
  const handleSendOTP = async (e: React.FormEvent) => {
    ...
    try {
      const res = await sendOTP(email, "client");
      if (res.success) {
        setOtpSent(true);
        if (res.code) {
  ...
  const handleVerifyOTP = async (e: React.FormEvent) => {
    ...
    try {
      const success = await verifyOTP(email, code, "client");
      if (success) {
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center p-4 bg-[#f9f6f0] overflow-hidden select-none">
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-[8%] left-[12%] w-[480px] h-[480px] rounded-full bg-gold-luxury/5 opacity-20 blur-[130px] animate-float-slow" />
        <div className="absolute bottom-[8%] right-[8%] w-[520px] h-[520px] rounded-full bg-purple-royal/6 opacity-15 blur-[140px] animate-float-reverse" />
        
        {/* Mandala Watermark */}
        <div className="absolute top-[50%] left-[50%] -translate-x-1/2 -translate-y-1/2 w-[700px] h-[700px] opacity-[0.02] text-purple-royal pointer-events-none animate-spin-slow">
      ...
      <GlassCard className="relative z-10 w-full max-w-md p-8 border-white/60 bg-white/40 shadow-2xl space-y-6">
        ...
        {!isSupabaseConfigured && (
          <div className="p-3 bg-purple-royal/5 border border-purple-royal/10 rounded-xl space-y-1 text-[10px] text-charcoal-dark/65 leading-relaxed">
            <p className="font-extrabold text-purple-royal uppercase tracking-wider flex items-center gap-1">
              <Compass size={12} /> Sandbox Mode Active
            </p>
            <p>Use any client email (e.g. <strong className="font-bold">client@gmail.com</strong>) to sign in. Enter any 6-digit OTP code to verify.</p>
          </div>
        )}
